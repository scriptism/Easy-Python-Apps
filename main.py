...
def simple_interest(principal, rate, time):
    ...
    rate is in percent per time unit; time must match that unit.
    """
...
# or
def check_preference2(preference):
    p = preference.lower()
    if p == "car":
        return "You like car"
    elif p == "bike":
...
# Splits on whitespace, drops leading/trailing spaces, returns empty list for blank strings
def string_to_list(s):
    return s.split()


print(string_to_list("Hello, world!"))        # ['Hello,', 'world!']
print(string_to_list("  Too   many spaces ")) # ['Too', 'many', 'spaces']
